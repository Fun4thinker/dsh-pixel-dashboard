 ...
 *   2) **需要授权 / 回答**由浏览器自己发现：DSH 把待办发布在
 *      `ctx.uiSession.pendingInteractions` 上（一个 `Map<SessionId, interaction>`），
 ...
 * 审批**，比普通提问更需要人回来看一眼。
 * @param {object} interaction - `uiSession.pendingInteractions` 里的一项。
 * @returns {{title:string,body:string,tag:string,level:string}} 文案。
 */
function composeInteraction(interaction) {
  ...
  // 待办总是挂在某条会话上（`pendingInteractions` 是 Map<SessionId, …>）。
...
const fetchPlans = m4.fetchPlans
const formatReset = m4.formatReset
      ...
      ? h('div', { className: 'px-rows' },
        snapshot.recent.slice(0, 8).map((item, index) => {
          ...
          return h(RecentRow, {
            ...
            value: `${item.title}${item.body === '' ? '' : ` — ${item.body}`}`,
            sessionId,
