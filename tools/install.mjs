/**
 * 安装脚本（早期「整机部署」方式，保留用于清理与兼容）。
 *
 * 新的标准装法是 `tools/install-official.mjs`（走官方 `dsh plugin add`）。
 * 本脚本仍可用于把插件文件铺进 `$DSH_HOME/plugins/` 并写 loader 行。
 *
 * 关键设计（都是踩过的坑）：
 *   1) **行 id 固定**。Loader 按行做增量更新；换成随版本变化的 id，会让新旧两行
 *      同时占用同一个 HTTP 路由，注册冲突导致整体回滚——而旧代码继续应答，
 *      表面上没有任何报错。
 *   2) **实现版本由源码哈希派生**，写在入口的动态 import 上。重新构建后版本号必变，
 *      因此**重启 dsh** 就会加载新实现（宿主侧代码只在启动时加载），
 *      不存在「忘了加版本号」这种失败模式。
 *      注意：Loader 的行级热更新只在 profile 开了 `patchReload: live` 且 HMR
 *      **模块重载**（`hmr` 行，dsh-base 里默认 `disabled: true`）时才生效；
 *      仅靠 `patchReload: live` 只重挂配置层，不会重新 import 宿主模块。
